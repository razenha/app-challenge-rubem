...
class InvoiceCanceledStrategy:
    def handle(self, event):
        ...
        with db.atomic():
            invoice.mark_as_canceled(stark_id=sb_invoice.id)
            invoice.save()

        logger.info(f"Invoice {invoice.stark_id} marked as CANCELED (local id={invoice.id})")
