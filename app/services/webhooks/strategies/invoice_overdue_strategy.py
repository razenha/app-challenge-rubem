...
class InvoiceOverdueStrategy:
    def handle(self, event):
        ...
        with db.atomic():
            invoice.mark_as_overdue(stark_id=sb_invoice.id)
            invoice.save()

        logger.info(f"Invoice {invoice.stark_id} marked as OVERDUE (local id={invoice.id})")
