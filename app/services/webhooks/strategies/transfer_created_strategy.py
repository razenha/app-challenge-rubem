...
class TransferCreatedStrategy:
    def handle(self, event):
        ...
        with db.atomic():
            transfer.mark_as_created(stark_id=sb_transfer.id)
            transfer.save()

        logger.info(f"Transfer {transfer.stark_id} marked as CREATED (local id={transfer.id})")
