...
class InvoiceExpiredStrategy:
    def handle(self, event):
        ...
        with db.atomic():
            invoice.mark_as_expired(stark_id=sb_invoice.id)
            invoice.save()

        logger.info(f"Invoice {invoice.stark_id} marked as EXPIRED (local id={invoice.id})")
