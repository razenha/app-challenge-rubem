...
class TransferProcessingStrategy:
    def handle(self, event):
        ...
        with db.atomic():
            transfer.mark_as_processing(stark_id=sb_transfer.id)
            transfer.save()

        logger.info(f"Transfer {transfer.stark_id} marked as PROCESSING (local id={transfer.id})")
